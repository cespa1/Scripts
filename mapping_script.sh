   ...
   for muestra in $(ls | grep .fq.gz| cut -d '_' -f1,2,3| uniq)
       do
           ...
           fastp -i $muestra*_1.fq.gz -o $muestra"_trimmed_dup_1.fq.gz" \
            ...
            --detect_adapter_for_pe --thread 23 -h $muestra".html"
...
do
    ...
    bowtie2 -x human_index -1 $muestra*trimmed_dup_1.fq.gz -2 $muestra*trimmed_dup_2.fq.gz -S $muestra".sam" --very-sensitive -X 500 -p 23 > $muestra"bowtie_log.txt"
...
for muestra in $(ls | grep _sort_dedup.bam | cut -d '_' -f1,2,3)
do
        macs2 callpeak -t $muestra*dedup.bam --outdir /home/scclab/Atacseq/fastq/peak_calling/$muestra"_ATAC_peaks.broadPeak" \
    -f BAM -n $muestra -g hs -q 0.0001 --nomodel --shift -37 --extsize 73 --broad

            macs2 callpeak -t $muestra*dedup.bam --outdir /home/scclab/Atacseq/fastq/peak_calling/$muestra"_ATAC_peaks.NarrowPeak" \
...
#for muestra in $(ls | grep _sort_dedup.bam | cut -d '_' -f1,2,3,4)
#do 
#htseq-count -n 20 --format=bam --type=exon --idattr=gene_name $muestra*.bam all_peaks_merged.gtf > ./htseq/$muestra"__htseq_counts"
#done
