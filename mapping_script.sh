...
echo "###########################    Empezando el preprocesamiento de los fastq    ###########################"
   for muestra in $(ls | grep .fq.gz| cut -d '_' -f1,2,3| uniq)
       do
...
do
    ...
    bowtie2 -x human_index -1 $muestra*trimmed_dup_1.fq.gz -2 $muestra*trimmed_dup_2.fq.gz -S $muestra".sam" -X 500 -p 23 > $muestra"bowtie_log.txt"
...
if [ $nombre_muestra == "humano" ]
then
for muestra in $(ls | grep _sort_dedup.bam | cut -d '_' -f1,2,3)
do
...
if [ $nombre_muestra == "humano" ]
then
for muestra in $(ls | grep _sort_dedup.bam | cut -d '_' -f1,2,3,4 | uniq)
do
        macs2 callpeak -t $muestra*dedup.bam --outdir /home/scclab/Atacseq/fastq/new_peak_calling/$muestra"_ATAC_peaks.broadPeak" \
    -f BAM -n $muestra -g hs -q 0.0001 --nomodel --shift -37 --extsize 73 --broad --broad-cutoff 0.01

            macs2 callpeak -t $muestra*dedup.bam --outdir /home/scclab/Atacseq/fastq/new_peak_calling/$muestra"_ATAC_peaks.NarrowPeak" \
    -f BAM -n $muestra -g hs -q 0.0001 --nomodel --shift -37 --extsize 73 
done
else
for muestra in $(ls | grep _sort_dedup.bam | cut -d '_' -f1,2,3)
do
...
touch /home/scclab/Atacseq/fastq/new_peak_calling/merge.bed
for muestra in $(ls | grep _sort_dedup.bam | cut -d '_' -f1,2,3,4)
do
cut -f1-4 /home/scclab/Atacseq/fastq/new_peak_calling/$muestra"_ATAC_peaks.broadPeak"/$muestra*"_peaks.broadPeak" > /home/scclab/Atacseq/fastq/new_peak_calling/$muestra"_ATAC_peaks.broadPeak"/$muestra"_peaks.bed"
cat /home/scclab/Atacseq/fastq/new_peak_calling/$muestra"_ATAC_peaks.broadPeak"/$muestra"_peaks.bed" >> /home/scclab/Atacseq/fastq/new_peak_calling/merge.bed
done

sort -k1,1 -k2,2n /home/scclab/Atacseq/fastq/new_peak_calling/merge.bed > /home/scclab/Atacseq/fastq/new_peak_calling/sorted_merge.bed

bedtools merge -i /home/scclab/Atacseq/fastq/new_peak_calling/sorted_merge.bed > /home/scclab/Atacseq/fastq/new_peak_calling/merge_final2.bed
#rm merge.bed

awk '{print $1"\tunknown\texon\t"$2"\t"$3"\t.\t+\t0\tgene_id \42"$1":"$2"-"$3"\42;\ttranscript_id \42"$1":"$2"-"$3"\42;\tgene_name \42"$1":"$2"-"$3"\42;"}' merge_final2.bed  > all_peaks_merged.gtf

for muestra in $(ls | grep _sort_dedup.bam | cut -d '_' -f1,2,3,4)
do 
htseq-count -n 20 --format=bam --type=exon --idattr=gene_name $muestra*.bam /home/scclab/Atacseq/fastq/new_peak_calling/all_peaks_merged.gtf > /home/scclab/Atacseq/fastq/new_htseq/$muestra"__htseq_counts"
done
