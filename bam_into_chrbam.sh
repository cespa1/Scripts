...
echo "Muestra high or low?"
read tipo_muestra
if [ $tipo_muestra == "high" ]
    then
        cd High
        for muestra in $(ls | grep .bam | cut -d '_' -f1,2,3,4 | uniq)
        do
            samtools view -H $muestra*".bam" |\
                ...
                sed -e 's/SN:MT/SN:chrM/' | samtools reheader - $muestra*".bam" > $muestra"_high.bam"
done
cd ..
else
    cd Low
    ...
    do
        samtools view -H $muestra*".bam" |\
            ...
            sed -e 's/SN:21/SN:chr21/' | sed -e 's/SN:22/SN:chr22/' | \
            sed -e 's/SN:X/SN:chrX/' | sed -e 's/SN:Y/SN:chrY/' | \
            sed -e 's/SN:MT/SN:chrM/' | samtools reheader - $muestra*".bam" > $muestra"_low.bam"
done
cd ..
fi